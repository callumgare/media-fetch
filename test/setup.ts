...
export default async function({ provide }: GlobalSetupContext) {
  ...
  if (typeof initialProxyServerAddress === "string" || initialProxyServerAddress === null) {
  ...
  return async () => {
    ...
    await new Promise<void>(
      (resolve, reject) => clientToCacheProxy.close(
...
function startClientToCacheProxy(cachingProxyServerAddress: string): http.Server<typeof http.IncomingMessage, typeof http.ServerResponse> {
