...
export const requestHandlerSchema = z.object({
  ...
  secretsSchema: z.custom<z.AnyZodObject>(val => val?.constructor?.name === 'ZodObject').optional(),
  paginationType: z.enum(["offset", "cursor", "none"]).default("none"),
  responses: z.array(
    z.object({
      requestMatcher: z.custom<z.AnyZodObject>(val => val?.constructor?.name === 'ZodObject').optional(),
      description: z.string().optional(),
      schema: z.custom<z.AnyZodObject>(val => val?.constructor?.name === 'ZodObject'),
